...
namespace NuLog.LogEvents
{
    ...
    // public DeferredLogEvent(Func<LogEvent> deferredFunc) { this.deferredLogEvent = new
    // Lazy<LogEvent>(() => { return deferredFunc(); }); }

    // public void WriteTo(ITarget target) { this.deferredLogEvent.Value.WriteTo(target); }
