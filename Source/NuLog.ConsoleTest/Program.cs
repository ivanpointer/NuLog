...
namespace NuLog.ConsoleTest
{
	internal class Program
	{
		...
		{
			int testTimes = GetIntFromUser("Number of test log events", 3);

			Stopwatch initializeSW = new Stopwatch();

			initializeSW.Start();
			LoggerFactory.Initialize(@"Configs\WindowsEventLog.json");
			LoggerBase logger = LoggerFactory.GetLogger("windowsEventLog", "windowsEventLog");
			initializeSW.Stop();
